    ...
    function NodeTool(name,className,description,defaultWidth,defaultHeight){
        ...
        this.mount = function(){
            ...
            $canvas.on("mouseup.nodeadd",function(ev){
                ...
                //if(this == ev.target){
                    that.getCanvas().createNode(that.getName(),ev.originalEvent.offsetX+offsetClick.left-offsetCanvas.left,ev.originalEvent.offsetY+offsetClick.top-offsetCanvas.top,_defaultWidth,_defaultHeight, that.getCanvas().getType());
