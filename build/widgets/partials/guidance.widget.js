    ...
    while (nodesToCheck.length > 0) {
      ...
      for (var i = 0; i < nodesToCheck.length; i++) {
        ...
        if (nodesInThread.indexOf(nodeId) >= 0) continue;
          ...
          if (node.type == "CONCURRENCY_NODE") ; else {
...
class ActivityStatus extends ConcurrentRegion {
    ...
    while (nodesToResolve.length > 0) {
      ...
      for (var i = 0; i < nodesToResolve.length; i++) {
        ...
        if (node.type == "SET_PROPERTY_ACTION") {
          ...
          nextNodesToResolve = nextNodesToResolve.concat(
            this.logicalGuidanceRepresentation.successors(nodeId)
          );
          ...
          var subActivity = new ActivityStatus(
            this.logicalGuidanceRepresentation,
            node.initialNodeId,
            this.strategy
          );
          ...
          var ingoing =
            this.logicalGuidanceRepresentation.predecessors(nodeId).length;
          var outgoing =
            this.logicalGuidanceRepresentation.successors(nodeId).length;
          if (outgoing > ingoing) {
            // Fork node
            if (!this.concurrentRegion)
              this.concurrentRegion = new ConcurrentRegion(
                this,
                this.logicalGuidanceRepresentation,
                nodeId
              );
            nextNodesToResolve.push(
              this.concurrentRegion.getCurrentThreadStart()
            );
          } else {
            // Join node
            if (
              this.concurrentRegion.isLastThread() &&
              this.concurrentRegion.isOwner()
            ) {
              ...
              if (nextThreadStart) nextNodesToResolve.push(nextThreadStart);
            }
          }
        } else {
          expectedNodes.push(nodeId);
  ...
  proceed(nodeId) {
    ...
    if (
      ...
      this.currentSubActivity.getExpectedNodes().indexOf(nodeId) >= 0
    ) {
...
class CollaborationStrategy extends GuidanceStrategy {
  ...
  init(logicalGuidanceRepresentation, space) {
    ...
    for (var i = 0; i < this.initialNodes.length; i++) {
      ...
      this.activityStatusList[Util.generateRandomId()] = new ActivityStatus(
        this.logicalGuidanceRepresentation,
        nodeId,
        this
      );
    ...
    this.iwc = IWCW.getInstance(CONFIG.WIDGET.NAME.GUIDANCE);
    this.iwc.registerOnDataReceivedCallback(
      this.onRevokeSharedActivityOperation,
      this
    );
    this.iwc.registerOnDataReceivedCallback(
  ...
  checkEdgeAddForActivity(id, type, activityStatus) {
    ...
    for (var i = 0; i < activityExpectedNodes.length; i++) {
      ...
      var node = this.logicalGuidanceRepresentation.node(nodeId);
      if (
        node.type == "CREATE_RELATIONSHIP_ACTION" &&
        node.relationshipType == type
      ) {
        return nodeId;
      }
    }
  ...
  onNodeAdd(id, type) {
    ...
    if (!nextNode) {
      ...
      for (var i = 0; i < this.initialNodes.length; i++) {
        var nodeId = this.initialNodes[i];
        var newActivity = new ActivityStatus(
          this.logicalGuidanceRepresentation,
          nodeId,
          this
        );
  ...
  onNodeDelete(id, type) {
    ...
    if (lastCreatedEntityId == id && this.currentActivity) {
      ...
      if (this.currentActivity) {
        if (
          !this.checkActivityValidityAfterNodeDelete(this.currentActivity, id)
        ) {
          this.currentActivity = null;
          updateGuidance = true;
        }
  ...
  showExpectedActions(entityId) {
    ...
    for (var activityId in this.sharedActivities) {
      ...
      if (this.sharedActivities[activityId].isOwner) continue;
  ...
  buildUi() {
    ...
    for (var i = 0; i < this.initialNodes.length; i++) {
      ...
      var node = this.logicalGuidanceRepresentation.node(nodeId);
      var listItem = $(
        "<li class='bs-list-group-item guidance-item'><p><i class='fa fa-puzzle-piece' style='margin-right:5px;'></i><span class='name'></span></p><p><small class='bs-text-muted description'></small></p></li>"
      );
      ...
      var tempActivity = new ActivityStatus(
        this.logicalGuidanceRepresentation,
        nodeId,
        this
      );
      ...
      listItem
        .find(".description")
        .text(
          this.getDescriptionTextForAction(expectedNodes[0]) +
  ...
  redrawHistoryList() {
    ...
    for (var i = this.activityHistory.length - 1; i >= 0; i--) {
      ...
      var node = this.logicalGuidanceRepresentation.node(nodeId);
      var listItem = $(
        "<li class='bs-list-group-item guidance-history-item' style='cursor: pointer;'><p><i class='fa fa-puzzle-piece' style='margin-right:5px;'></i><span class='name'></span></p><p><small class='bs-text-muted description'></small></p></li>"
      );
      ...
      listItem.click(function () {
        ...
        var operation = new MoveCanvasOperation(
          that.currentActivity.lastAddedNode,
          false
        );
        that.iwc.sendLocalNonOTOperation(
          CONFIG.WIDGET.NAME.MAIN,
          operation.toNonOTOperation()
        );
    ...
    switch (data.operationType) {
      ...
      case "CollaborationStrategy:UpdateSharedActivity":
        ...
        if (this.sharedActivities.hasOwnProperty(activityId)) {
          this.sharedActivities[activityId].removeThreadFromConcurrentRegion(
            data.removedThreadId
          );
        }
        break;
    }
...
(function () {
        ...
        if (
          ...
          d.send(function (c) {
            ...
            "object" === typeof c &&
              null !== c &&
              "function" === typeof c.getId &&
              ((c = c.getId()), "string" === typeof c && (ownData.viewer = c));
            g.publish(a, b);
          });
          return;
        }
      ...
      function (c) {
        ...
        if (c.oauthApprovalUrl) {
          ...
          c = {
            createOpenerOnClick: function () {
              return function () {
                if ((n = window.open(t, "_blank", "width=450,height=500"))) {
                  (q = window.setInterval(s, 100)),
                    (document.getElementById("oauthPersonalize").style.display =
                      "none"),
                    (document.getElementById(
                      "oauthPersonalizeDone"
                    ).style.display = "block");
          ...
          document.all
            ...
            : (document.getElementById("oauthPersonalizeMessage").textContent =
                f);
          document.getElementById("oauthPersonalize").style.display = "block";
        } else {
          c.oauthError
      ...
      openapp.io.makeRequest(
        ...
        function (a) {
          ...
          a.headers.hasOwnProperty("content-location") &&
            (b.contentUri = a.headers["content-location"][0]);
          a.headers.hasOwnProperty("content-type") &&
            "application/json" === a.headers["content-type"][0].split(";")[0] &&
            (b.data = gadgets.json.parse(b.data));
          b.subject =
            "undefined" !== typeof a.data
              ? b.data.hasOwnProperty("")
    ...
    return {
      ...
      followSeeAlso: function () {
        ...
        if ("undefined" !== typeof b) {
          ...
          for (
            e = 0;
            e < b.length && e < a.uri.length && b.charAt(e) === a.uri.charAt(e);
            e++
          ) {
            "/" === b.charAt(e) && d++;
          }
        ...
        var b =
            null != a.type
              ? openapp.resource
                  ...
                  .type(a.type)
                  .list()
              : openapp.resource.context(this.context).sub(a.relation).list(),
          d = [],
          c = 0;
        c < b.length;
        c++
  ...
  OARP.followReference = function (a) {
    this._referenceLoaded
      ...
      : this._call(function () {
          var b =
            this.context.data[this.uri]["http://www.w3.org/2002/07/owl#sameAs"];
          null != b && 0 < b.length
            ? a(new openapp.oo.Resource(b[0].value))
            : a(this);
        });
...
let GuidanceWidget = class GuidanceWidget extends SyncMetaWidget(LitElement, getWidgetTagName(CONFIG.WIDGET.NAME.GUIDANCE)) {
    firstUpdated(_changedProperties) {
        super.firstUpdated(_changedProperties);
        yjsSync()
            .then((y) => {
            window.y = y;
            console.info("GUIDANCE: Yjs Initialized successfully in room " +
                window.spaceTitle +
                " with y-user-id: " +
                y.clientID);
...
GuidanceWidget = __decorate([
    e(getWidgetTagName(CONFIG.WIDGET.NAME.GUIDANCE))
], GuidanceWidget);
function initGuidanceWidget(y) {
    ...
    var selectedStrategy = new strategies[0](dataMap.get("guidancemodel"), Space$1);
    selectedStrategy.buildUi();
    var $strategies = $("#guidanceSelect");
    for (var i = 0; i < strategies.length; i++) {
        var strategy = strategies[i];
        var $strategy = $(lodash.template("<li><a href='javascript:;'><i class='fa fa-${icon}' style='margin-right:5px;'></i>${name}</a></li>")({ name: strategy.NAME, icon: strategy.ICON }));
    ...
    var initStrategy = function (index) {
        const dataMap = y.getMap("data");
        selectedStrategy = new strategies[index](dataMap.get("guidancemodel"), Space$1);
...
export { GuidanceWidget };
//# sourceMappingURL=guidance.widget.js.map
