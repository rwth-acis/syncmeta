...
function GhostEdgeGuidance(canvas, node1, node2) {
  ...
  this.remove = function () {
    _button.detach();
    if (_currentEdge) _currentEdge.remove();
  };
  ...
  this.setCurrentEdge = function (edge) {
    ...
    createEdgeButton.click(function (event) {
      ...
      //else {
      _canvas.createEdge(
        _currentEdge.getEdgeFunction().getType(),
        _currentEdge.getSource().getEntityId(),
        _currentEdge.getTarget().getEntityId()
      );
      //}
...
export default GhostEdgeGuidance;
