...
import { default as SingleColorValueAttribute } from "./SingleColorValueAttribute";
  ...
  constructor(
    ...
    this.bindMoveToolEvents = function () {
      ...
      jsPlumbInstance.bind(EVENT_DRAG_START, (params) => {
        ...
        // this._$node.resizable({ disabled: true });
        drag = false;
      });

      jsPlumbInstance.bind(EVENT_DRAG_STOP, (params) => {
        that._$node.css({ opacity: "" });
        // this._$node.resizable("enable");
        _canvas.bindMoveToolEvents();
        var id = that._$node.attr("id");
        //this._$node.css({top: originalPos.top / _canvas.getZoom(), left: originalPos.left / _canvas.getZoom()});
        const x = that._$node.position().left;
        const y = that._$node.position().top;
      ...
      jsPlumbInstance.bind(EVENT_DRAG_MOVE, () => {
        _canvas.hideGuidanceBox();
      });
      ...
      var viewOnly = widgetConfigMap.get("view_only");
      if (viewOnly) {
        this._$node.on("click").draggable().draggable("destroy");
        this._$node.on("click").contextMenu(false);
      }
    };
    ...
    this.unbindEdgeToolEvents = function () {
      try {
        this._$node.removeClass("source target");
