...
function getSpaceInfo(f) {
  new openapp.oo.Resource(openapp.param.space()).getInfo(function (space) {
    f(
      space["http://purl.org/dc/terms/title"],
      space["http://purl.org/dc/terms/description"]
    );
  });
}
/*
...
function ILDE(username, password, resource) {
  ...
  $.ajax({
    ...
    data:
      '<?xml version="1.0" encoding="UTF-8"?><login><username>' +
      username +
      "</username><password>" +
      password +
      "</password></login>",
  ...
  $.ajax({
    ...
    success: function () {
      try {
        ...
        window.sayError(
          "Sorry, this ILDE installation did not return a proper design."
        );
        throw new Error(
          "The returned json object is not valid! (ILDE getLdsDataById)"
        );
      }
    },
    error: function () {
...
ILDE.prototype.newLds = function (design, zip, parentid, callback) {
  ...
  getSpaceInfo(function (space_title, space_description) {
    ...
    var properties =
      ...
      "<license>1</license>";
    if (parentid != null) {
      properties = properties + "<parent>" + parentid + "</parent>";
    }
    properties = properties + "</lds>";
    form.append(
      "properties",
      new Blob([properties], { type: "application/xml" }, "myproperties.xml")
    );
    form.append(
      "design",
      new Blob([design], { type: "application/octet-stream" }, "mydesign.glm")
    );
    if (zip != null) {
      // because of a bug in imsld we must not send the zip.
