    ...
    describe("EntityManager meta modeling init test", function () {
      ...
      describe("Create node & edge test", function () {
        ...
        var objectNode = null,
          ...
          biDirAsso;
        before(function (done) {
          objectNode = EntityManager.createNode(
            "Object",
            "1234567890",
            4000,
            4000,
          ...
          enumNode = EntityManager.createNodeFromJSON(
            json.type,
            id,
            json.left,
            json.top,
          ...
          biDirAsso = EntityManager.createEdge(
            "Bi-Dir-Association",
            "1234567891",
            objectNode,
            enumNode
        ...
        it("Create Object node test", function () {
          ...
          expect(objectNode.get$node().attr("class")).to.be.equal(
            "node object"
          );
        ...
        it("Create Enum node from JSON test", function () {
          ...
          expect(attributes.constructor.name).to.be.equal(
            "SingleValueListAttribute"
          );
          expect(attributes.getName()).to.be.equal(
            json.attributes["[attributes]"].name
          );
          ...
          for (var attrKey in attrList) {
            if (attrList.hasOwnProperty(attrKey)) {
              ...
              expect(attr.getValue().getValue()).to.be.equal(
                attrList[attrKey].value.value
              );
            }
          }
        });
        ...
        it("Find object node test", function () {
          ...
          expect(EntityManager.getNodes().hasOwnProperty("1234567890")).to.be
            .true;
          expect(
            EntityManager.getNodesByType("Object").hasOwnProperty("1234567890")
          ).to.be.true;
          expect(
            _.keys(EntityManager.getNodesByType("Object")).length
          ).to.be.equal(1);
        });
        ...
        it("Find enum node test", function () {
          ...
          expect(EntityManager.getNodesByType("Enumeration").hasOwnProperty(id))
            .to.be.true;
          expect(
            _.keys(EntityManager.getNodesByType("Enumeration")).length
          ).to.be.equal(1);
        });
        ...
        it("Find Bi-Dir edge test", function () {
          ...
          expect(
            EntityManager.getEdgesByType("Bi-Dir-Association").hasOwnProperty(
              "1234567891"
            )
          ).to.be.true;
        });
        ...
        it("Check object node connections and neighbors", function () {
          expect(objectNode.getOutgoingEdges().hasOwnProperty("1234567891")).to
            .be.true;
          expect(objectNode.getIngoingEdges().hasOwnProperty("1234567891")).to
            .be.false;
          expect(objectNode.getNeighbors().hasOwnProperty(id)).to.be.true;
        });

        it("Check enum node connections and neightbors", function () {
          expect(enumNode.getOutgoingEdges().hasOwnProperty("1234567891")).to.be
            .false;
          expect(enumNode.getIngoingEdges().hasOwnProperty("1234567891")).to.be
            .true;
          expect(enumNode.getNeighbors().hasOwnProperty("1234567890")).to.be
            .true;
        });
        ...
        describe("Delete node & edges tests", function () {
          ...
          it("Bi-Dir-Association edge should no longer be in EntityManager", function () {
            expect(EntityManager.getEdges().hasOwnProperty("1234567891")).to.be
              .false;
      ...
      describe("Create ModelAttributeNode tests", function () {
        ...
        it("Check Properties", function () {
          expect(modelAttributeNode.getEntityId()).to.be.equal(
            "modelAttributes"
          );

          var attributes = modelAttributeNode.getAttributes();
          expect(attributes.hasOwnProperty("modelAttributes[name]")).to.be.true;
          expect(attributes.hasOwnProperty("modelAttributes[description]")).to
            .be.true;
          expect(
            modelAttributeNode.getLabel().getValue().getValue()
          ).to.be.equal("Model Attributes");
          expect(modelAttributeNode.get$node().length).to.be.equal(1);
          expect(
            modelAttributeNode.get$node().find(".label").text()
          ).to.be.equal("Model Attributes");
        ...
        describe("Delete ModelAttributesNode test", function () {
          ...
          it("Check Deletion", function () {
            expect(EntityManager.createModelAttributesNode()).to.be.not.equal(
              modelAttributeNode
            );
          });
        });
...
export default EntityManagerTests;
